...
- **Модульные слоты (усиление E2)**: `config.modules: string[]` — до 3 модулей-MCP, вставляются в ConfigPanel (docs/06). `MODULE_DEFS` в `core/nodes/modules.ts`: `{ id, label, energyCost }` — `web-search` (веб-поиск посреди хода, через сервер, docs/07) и `memory` (снапшот содержимого сундуков подмешивается в prompt как RAG-контекст; движок передаёт его в NodeCtx). Каждый модуль повышает расход энергии станка (docs/04).
  - **Осознанное исключение из инварианта манипулятора (docs/03)**: `memory` читает буферы chest напрямую — `Engine.collectChestMemory()` в `engine.ts` перебирает `this.buffers` всех chest и отдаёт их содержимое в `ctx.memory`, БЕЗ edge и БЕЗ манипулятора. Это не баг и не дырка в графе доставки: assembler ничего не «выгружает» через эту границу, он лишь читает read-only снапшот уже накопленного в буфере (сами данные туда попали через обычный manipulator-gated edge). Явно зафиксировано здесь, чтобы этот путь не «переоткрывали» повторно как обход инварианта — при необходимости менять семантику manipulator-required delivery эту read-only ветку трогать не нужно.
