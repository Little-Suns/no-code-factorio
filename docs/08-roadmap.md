...
- [ ] **E1. Электричество** (~3ч): аккумулятор (2×2, заряд/ёмкость = токены, кнопка «Зарядить», полоска заряда на спрайте), списание в Engine, шкала в TopBar, «Нет питания» у станка. Питание глобальное, без столбов (docs/04).
...
- [ ] **E4. Энергостолбы + чертежи** (~5ч): столбы `pole`, провода-визуал от аккумулятора (BFS-связность, покрытие 5×5), «вне линии = обесточен»; чертежи (рамка, localStorage, групповой ghost, экспорт строкой). Брать, только если E1–E3 стабильны (docs/04, 03, 06, 02).
