...
- [ ] **E1. Энергия упрощённая** (~3ч): один ресурс в store, списание в Engine, полоска в TopBar, «нет питания» у станка (docs/04).
...
- [ ] **E4. Энергия полная + чертежи** (~6ч): ток/топливо, браунаут-замедление, «подкинуть угля»; чертежи (рамка, localStorage, групповой ghost, экспорт строкой). Столбы с радиусом — только при большом запасе времени. Брать, только если E1–E3 стабильны (docs/04, 03, 06).
