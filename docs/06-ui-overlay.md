...
Зажим+растягивание ЛКМ без активного инструмента/чертежа на кисти → рамка выделения (рисует Pixi ghost-слой в `game/input.ts`; клик без движения ЛКМ — это по-прежнему обычное выделение станка, отличаются по пиксельному сдвигу курсора, `DRAG_THRESHOLD`) → станки внутри рамки подсвечиваются отдельным Graphics-оверлеем (пока идёт драг и пока открыта форма сохранения); `Del`/`Backspace` при открытой форме сохранения сносит разом все захваченные рамкой станки (`store.removeMany`) → «Сохранить чертёж» (имя) → список в `ui/BlueprintPanel.tsx` — самостоятельная плавающая панель (слева внизу, зеркало `ResultPanel` по вёрстке), не вкладка Hotbar, как планировалось изначально. Видимость панели — `store.blueprintPanelOpen` (переключатель по `B`/`И`, дефолт `false`, не персистится) **или** активный `pendingSelection` — рамка сама открывает панель, чтобы форму сохранения было видно даже без явного `B`. Постановка — групповой ghost, кисть чертежа не сбрасывается после клика (аналогично `selectedTool`). Export/Import чертежа строкой (base64) — обычные контролируемые инпуты в панели, без `window.prompt`/диалогов (в проекте нигде не используются нативные диалоги).
