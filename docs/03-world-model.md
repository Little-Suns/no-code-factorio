...
`accumulator` и `pole` (усиление) — энергослой: занимают тайлы, но портов не имеют и в трассировке лент не участвуют, `buildGraph` их игнорирует. Запитанность станков считается отдельно (docs/04).
