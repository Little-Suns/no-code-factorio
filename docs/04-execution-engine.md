...
class Engine {
  ...
  setPaused(paused: boolean): void    // видимость вкладки — паузит источники, не трогает уже летящее
  setDebugMode(enabled: boolean): void // пошаговая отладка (см. ниже)
  step(): void                        // пропустить ровно одно ожидающее событие
}
```

### Пошаговая отладка (кнопки Пауза/Шаг в TopBar)

`setDebugMode(true)` держит источники новых пакетов (интервал-шахты, вебхуки — как и `paused`,
но отдельный флаг: `paused` дёргается по `visibilitychange` и не должен тихо снимать дебаг-паузу
при сворачивании вкладки) и ставит каждое значимое пакет-событие на «ворота» (`debugGate`,
приватный): spawn у шахты (перед вызовом handler'а, т.е. и сетевые `mode=url` тоже ждут),
spawn каждого клона на выходе станка (дублер/сплиттер — отдельный шаг на каждую копию), consume
на входе в обычный станок/chest/mixer. Пакет, доехавший визуально по ленте (`transport.move`
уже отработал), «не входит» в станок, пока не пришёл `step()`. `step()` пропускает РОВНО одно
ожидающее событие (FIFO; если в этот момент никто не ждёт — кредит копится и «съедается»
следующим, которое встанет на ворота). `setDebugMode(false)`/`stop()` отпускают все ворота разом.
Ручной `triggerMiner()` (кнопка «▶ Вбросить» в ConfigPanel) работает независимо от паузы — это
единственный способ подать пакет вручную во время пошаговой отладки. `__checks__/engine.ts`
AC17 — эталонный сценарий miner→assembler→silo, ровно 4 ворот на весь путь.
