...
## Энергия (усиление, не MVP)

**E1 — упрощённая (один ресурс):** `store.energy = { budget, used }` (budget задаётся в TopBar). Перед каждым LLM-вызовом движок списывает оценку `~(prompt.length / 4 + 400) × (1 + модули × 0.5)`; не хватает — `node-status error 'Нет энергии'`, пакет ждёт в очереди, повтор каждые 2 с до пополнения или Stop.

**E4 — полная (два ресурса, только поверх стабильного E1):**
- **Ток** = rate-limit API: восстанавливается со временем (`+N в секунду до максимума`). Нехватка → **браунаут**: станки не встают, а работают медленнее (движок добавляет задержку ×2–3 перед handler; рендер замедляет work-анимацию) — честная модель бэкоффа при упоре в rate-limit.
- **Топливо** = денежный бюджет: конечный запас, расходуется на каждый вызов. Кончился → станки мигают «нет топлива» и стоят намертво, пока игрок не «подкинет угля» (кнопка пополнения в TopBar).
- Энергостолбы с радиусом действия (новая сущность, пространственная головоломка) — часть E4, дорого: брать только при большом запасе времени.
