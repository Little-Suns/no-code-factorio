 ...
 * библиотек — правило проекта) и короткие описания станков для тултипа.
 * Тексты описаний — сжатый пересказ docs/05-nodes-catalog.md, ничего не
 * придумано заново.
 */
...
// Короткое описание роли станка — источник: docs/05-nodes-catalog.md.
export const TOOL_DESCRIPTIONS: Record<MachineKind, string> = {
  belt: 'Конвейер — переносит пакет на соседний тайл по направлению.',
  miner: 'Шахта — добывает сырьё: заданный текст, содержимое URL или тело вебхука.',
  assembler: 'Сборочный станок (Агент) — обрабатывает текст через LLM по выбранному рецепту.',
  splitter: 'Разветвитель — делит поток на true/false по JS-условию или ответу LLM.',
  mixer: 'Химзавод / Смеситель — склеивает или LLM-синтезирует несколько входящих пакетов в один.',
  silo: 'Ракета (Output) — принимает финальный результат и открывает панель результатов.',
  furnace: 'Печь — быстрый JS-препроцессор без LLM (например, чистит текст от HTML-тегов).',
  chest: 'Сундук — копит пакеты в буфер и отдаёт их пачкой заданного размера дальше.',
  lab: 'Лаборатория (Критик) — проверяет результат по критериям: PASS пропускает, REWORK возвращает на доработку.',
  accumulator: 'Аккумулятор — хранит заряд (токены LLM) и питает станки; без него электричество отключено.',
  webhook: 'Антенна / Webhook — отправляет данные на внешний HTTP-адрес (Discord, Slack, GitHub, свой сервер).',
  manipulator: 'Манипулятор — обязательный переходник между станками: без него станок↔станок и лента↔станок не соединяются.',
};
