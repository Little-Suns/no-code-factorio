...
const ru: Dict = {
  ...
  'tutorial.step.welcome.desc': 'Здесь ты строишь конвейер из AI-станков: шахта добывает данные, станки обрабатывают их через LLM, лента везёт результат к ракете. Короткий тур из 15 шагов, часть — практика прямо на карте. Можно пропустить в любой момент.',
  ...
  'tutorial.step.manipulator.desc': 'Попробуй прямо сейчас: поставь любые два станка и соедини их лентой и манипулятором (манипулятор — на границе, куда упирается лента). Без манипулятора связь просто не построится. Как только образуется рабочая связь, шаг сам отметится готовым.',
  ...
  'tutorial.step.move.desc': 'Зажми ЛКМ на станке и потяни — он переедет вместе с курсором. Так же работает и с целой группой: растяни рамку выделения по нескольким станкам, а затем зажми и потяни любой из них. Попробуй сдвинуть хоть что-нибудь.',
  ...
  'tutorial.step.blueprintStamp.desc': 'Нажми «Поставить» у своего чертежа или у пресета из библиотеки, затем кликни по карте — так копируется сразу целая группа станков. (Кнопка «Экспорт строкой» рядом копирует чертёж в буфер обмена, чтобы поделиться им.)',
  ...
  'tutorial.step.debug.desc': 'Кнопка «Пауза» рядом с Run замораживает фабрику на месте, а «Шаг» продвигает её ровно на один тик — удобно смотреть, как пакет проходит через конвейер шаг за шагом. Если что-то всё равно не едет — загляни в Логи, там видны тупики и ошибки.',
...
const en: Dict = {
  ...
  'tutorial.step.welcome.desc': 'You build a pipeline of AI machines: a miner produces data, machines process it through an LLM, belts carry the result to a rocket. A quick 15-step tour — some steps are hands-on practice. Skip anytime.',
  ...
  'tutorial.step.manipulator.desc': "Try it now: place any two machines and connect them with a belt and a manipulator (the manipulator goes right where the belt meets the machine). Without it the connection won't form. Once a working link exists, this step marks itself done.",
  ...
  'tutorial.step.move.desc': 'Press and hold LMB on a machine, then drag — it follows the cursor. Works on a whole group too: drag a selection box over several machines, then press-hold-drag any one of them. Try moving something.',
  ...
  'tutorial.step.blueprintStamp.desc': 'Hit "Stamp" on one of your blueprints or a library preset, then click the map — that drops a whole copy of the group at once. (The "Export as string" button next to it copies a shareable blueprint string to your clipboard.)',
  ...
  'tutorial.step.debug.desc': 'The Pause button next to Run freezes the factory in place, and Step advances it exactly one tick at a time — handy for watching a packet move through the pipeline. If things still aren\'t moving, check the Logs panel for dead ends and errors.',
...
const zh: Dict = {
  ...
  'tutorial.step.welcome.desc': '在这里搭建 AI 机器流水线：矿机产出数据，机器通过 LLM 处理数据，传送带把结果送到火箭。快速十五步导览，其中几步要在地图上亲手操作，随时可以跳过。',
  ...
  'tutorial.step.manipulator.desc': '现在就试试：放置任意两台机器，用传送带和机械臂把它们连起来（机械臂放在传送带对接机器的地方）。没有它连接建立不起来。一旦连接生效，这一步会自动标记完成。',
  ...
  'tutorial.step.move.desc': '在机器上按住鼠标左键拖动，它就会跟着光标移动；对整组机器同样有效——先拖出选框圈住多台机器，再按住其中任意一台拖动即可。试着移动点什么。',
  ...
  'tutorial.step.blueprintStamp.desc': '点击你的蓝图或蓝图库预设上的「放置」，再点击地图——就能一次性复制出整组机器。（旁边的「导出为字符串」按钮会把蓝图复制到剪贴板，方便分享。）',
  'tutorial.step.logs.title': '日志',
  'tutorial.step.logs.desc': '机器错误和系统事件会汇总在这里。',
  'tutorial.step.debug.title': '单步调试',
  'tutorial.step.debug.desc': 'Run 按钮旁边的「暂停」会让工厂原地冻结，「单步」则每次仅推进一个节拍——适合逐步观察包裹如何流经生产线。如果东西还是不动，再去日志面板看看死路和错误。',
