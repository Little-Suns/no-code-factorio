...
const ru: Dict = {
  ...
  'tutorial.step.welcome.title': 'Что это за инструмент',
  'tutorial.step.welcome.desc': 'No-code платформа для визуализации работы кодовых и агентных систем: без единой строчки кода собираешь систему и смотришь, как она работает изнутри.',
  'tutorial.step.metaphor.title': 'Завод — это метафора',
  'tutorial.step.metaphor.desc': 'Собирая завод, ты строишь рабочую систему агентов: объекты едут от станка к станку так же, как пакеты данных — между сервисами и протоколами под капотом. Это и есть обучение: на практике, а не по схемам.',
  'tutorial.step.terms.title': 'Три слова, которые тут всюду',
  'tutorial.step.terms.desc': 'Станок — обрабатывает данные, как функция. Конвейер (он же лента) — везёт данные между станками, как очередь. Манипулятор — вручную передаёт данные со станка на станок: сам по себе конвейер этого не умеет.',
  'tutorial.step.hotbar.title': 'Панель станков',
  'tutorial.step.hotbar.desc': 'Внизу — инструменты по группам: логистика (лента, манипулятор) слева, станки справа. Клик или клавиша выбирает инструмент, повтор или Esc — снимает выбор.',
  'tutorial.step.place.title': 'Поставь что-нибудь',
  'tutorial.step.place.desc': 'Выбери инструмент и кликни по карте — станок встанет на этом месте. Лента ставится протяжкой мышью, Delete сносит выделенное. Попробуй прямо сейчас.',
  'tutorial.step.manipulator.title': 'Манипулятор — вот как соединять станки',
  'tutorial.step.manipulator.desc': 'Станки не передают данные друг другу напрямую — это умеет только манипулятор. Повтори схему ниже: поставь два станка, протяни между ними ленту, а на границе у станка-приёмника поставь манипулятор стрелкой внутрь него (не туда — выдели манипулятор и нажми R).',
  'tutorial.diagram.machine': 'станок',
  'tutorial.diagram.belt': 'лента',
  'tutorial.diagram.manipulator': 'манипулятор →',
  'tutorial.step.rotate.title': 'Поворот станка',
  'tutorial.step.rotate.desc': 'Выдели уже поставленный станок кликом и нажми R — он повернётся на 90°. Попробуй прямо сейчас, шаг засчитается сам.',
  'tutorial.step.move.title': 'Перемещение станков',
  'tutorial.step.move.desc': 'Зажми ЛКМ на станке и потяни. Работает и с группой: выдели рамкой несколько станков, затем зажми и потяни любой из них. Попробуй сдвинуть хоть что-нибудь.',
  ...
  'tutorial.step.blueprints.desc': 'Растяни рамку выделения ЛКМ по нескольким станкам на карте — здесь, в этой панели, появится форма сохранения. Дай чертежу имя и сохрани.',
  'tutorial.step.blueprintStamp.title': 'Постановка чертежа',
  'tutorial.step.blueprintStamp.desc': 'Нажми «Поставить» у чертежа или у пресета из библиотеки, затем кликни по карте — скопируется сразу вся группа станков. «Экспорт строкой» рядом копирует чертёж в буфер обмена, чтобы поделиться им.',
  ...
  'tutorial.step.debug.desc': 'Кнопка «Пауза» рядом с Run замораживает фабрику на месте, а «Шаг» продвигает её ровно на один тик — удобно смотреть путь пакета. Если что-то всё равно не едет — загляни в Логи.',
...
const en: Dict = {
  ...
  'tutorial.step.welcome.title': 'What this actually is',
  'tutorial.step.welcome.desc': "A no-code platform for visualizing how code and agent systems work: you build a system without writing a line of code, and watch it run from the inside.",
  'tutorial.step.metaphor.title': 'The factory is a metaphor',
  'tutorial.step.metaphor.desc': "Building a factory means building a working agent system: objects travel from machine to machine the same way data packets move between services and protocols under the hood. That's the whole point — learning by doing, not by reading diagrams.",
  'tutorial.step.terms.title': "Three words you'll see everywhere",
  'tutorial.step.terms.desc': 'A machine processes data, like a function. A belt (aka conveyor) moves data between machines, like a queue. A manipulator hands data from one machine to another by hand — a belt alone can\'t do that.',
  'tutorial.step.hotbar.title': 'Machine bar',
  'tutorial.step.hotbar.desc': "Tools live down here in two groups: logistics (belt, manipulator) on the left, machines on the right. Click or press a key to pick one; click again or Esc to deselect.",
  'tutorial.step.place.title': 'Place something',
  'tutorial.step.place.desc': 'Pick a tool and click the map — a machine appears there. Drag to lay a belt, Delete removes the selected one. Go ahead and try it now.',
  'tutorial.step.manipulator.title': "Manipulator — here's how to connect machines",
  'tutorial.step.manipulator.desc': "Machines can't hand items to each other directly — only a manipulator does that. Recreate the diagram below: place two machines, drag a belt between them, then place a manipulator at the receiving machine with its arrow pointing INTO it (wrong way? select it and press R).",
  'tutorial.diagram.machine': 'machine',
  'tutorial.diagram.belt': 'belt',
  'tutorial.diagram.manipulator': 'manipulator →',
  'tutorial.step.rotate.title': 'Rotating a machine',
  'tutorial.step.rotate.desc': 'Select a machine you already placed and press R — it spins 90°. Go ahead and try it, the step completes on its own.',
  'tutorial.step.move.title': 'Moving machines',
  'tutorial.step.move.desc': 'Press and hold LMB on a machine, then drag. Works on a group too: drag a selection box over several machines, then press-hold-drag any one of them. Try moving something.',
  ...
  'tutorial.step.blueprints.desc': 'Drag a selection box with LMB over some machines on the map — a save form shows up right here in this panel. Name it and save.',
  'tutorial.step.blueprintStamp.title': 'Stamping a blueprint',
  'tutorial.step.blueprintStamp.desc': 'Hit "Stamp" on a blueprint or a library preset, then click the map — that drops a whole copy of the group at once. "Export as string" next to it copies a shareable blueprint string to your clipboard.',
  ...
  'tutorial.step.debug.desc': "The Pause button next to Run freezes the factory in place, and Step advances it one tick at a time — handy for watching a packet's path. Still stuck? Check the Logs panel.",
...
const zh: Dict = {
  ...
  'tutorial.next': '下一步 →',
  'tutorial.finish': '完成',
  'tutorial.step.welcome.title': '这到底是什么',
  'tutorial.step.welcome.desc': '一个用于可视化代码系统和智能体系统如何运作的 no-code 平台：不写一行代码就能搭建系统，并且亲眼看着它在内部运转。',
  'tutorial.step.metaphor.title': '工厂只是一个比喻',
  'tutorial.step.metaphor.desc': '搭建工厂，其实就是在搭建一套真正运转的智能体系统：物体在机器之间流转，正如数据包在底层的服务与协议之间流转一样。这就是学习的方式——靠动手，而不是靠看图。',
  'tutorial.step.terms.title': '几个反复出现的词',
  'tutorial.step.terms.desc': '机器——处理数据，就像一个函数。传送带——在机器之间搬运数据，就像一个队列。机械臂——手动把数据从一台机器交给另一台，传送带本身做不到这一点。',
  'tutorial.step.hotbar.title': '机器栏',
  'tutorial.step.hotbar.desc': '下方的工具分为两组：左边是物流（传送带、机械臂），右边是机器。点击或按键选择工具，再次点击或按 Esc 取消选择。',
  'tutorial.step.place.title': '放一个试试',
  'tutorial.step.place.desc': '选中一个工具，点击地图——机器就会出现在那里。拖拽鼠标铺设传送带，Delete 拆除选中项。现在就试试。',
  'tutorial.step.manipulator.title': '机械臂 — 这样连接机器',
  'tutorial.step.manipulator.desc': '机器之间不能直接传递物品，只有机械臂可以做到。照着下面的示意图操作：放置两台机器，在它们之间拖出一条传送带，再在接收机器旁放置机械臂，箭头指向它（方向不对就选中它按 R 旋转）。',
  'tutorial.diagram.machine': '机器',
  'tutorial.diagram.belt': '传送带',
  'tutorial.diagram.manipulator': '机械臂 →',
  'tutorial.step.rotate.title': '旋转机器',
  'tutorial.step.rotate.desc': '点选一台已放置的机器，按 R 键——它会旋转 90°。现在就试试，完成后这一步会自动通过。',
  'tutorial.step.move.title': '移动机器',
  'tutorial.step.move.desc': '在机器上按住鼠标左键拖动即可。对整组机器同样有效：先拖出选框圈住多台机器，再按住其中任意一台拖动。试着移动点什么。',
  ...
  'tutorial.step.blueprints.desc': '按住鼠标左键在地图上拖出选框圈住几台机器——保存表单会直接出现在这个面板里，起个名字保存即可。',
  'tutorial.step.blueprintStamp.title': '放置蓝图',
  'tutorial.step.blueprintStamp.desc': '点击蓝图或蓝图库预设上的「放置」，再点击地图——就能一次性复制出整组机器。旁边的「导出为字符串」按钮会把蓝图复制到剪贴板，方便分享。',
  'tutorial.step.logs.title': '日志',
  'tutorial.step.logs.desc': '机器错误和系统事件会汇总在这里。',
  'tutorial.step.debug.title': '单步调试',
  'tutorial.step.debug.desc': 'Run 按钮旁边的「暂停」会让工厂原地冻结，「单步」则每次仅推进一个节拍，方便观察包裹的路径。还是卡住了？去日志面板看看。',
